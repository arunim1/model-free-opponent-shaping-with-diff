...
def get_log(
    ...
    for i in range(num_steps):
        ...
        log["params_p1"].append(
            probs[0].detach()[runs_to_track].squeeze().cpu().numpy().tolist()
        )
        log["params_p2"].append(
            probs[1].detach()[runs_to_track].squeeze().cpu().numpy().tolist()
        )
        log["avg_Ms"].append(M.detach().mean(dim=0).squeeze().cpu().tolist())
        log["Ms"].append(M.detach()[runs_to_track].squeeze().cpu().numpy().tolist())
        log["avg_rewards_p1"].append(r_p1.detach().mean(dim=0).squeeze().cpu().tolist())
        log["avg_rewards_p2"].append(r_p2.detach().mean(dim=0).squeeze().cpu().tolist())
        log["rewards_p1"].append(r_p1.detach()[runs_to_track].squeeze().cpu().tolist())
        log["rewards_p2"].append(r_p2.detach()[runs_to_track].squeeze().cpu().tolist())
